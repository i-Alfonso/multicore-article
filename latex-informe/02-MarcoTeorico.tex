...
El operador Prewitt~\cite{gonzalez2008,prewitt1970} es un operador diferencial
...
Un aspecto crucial para que el compilador pueda vectorizar
autom\'aticamente un bucle es la ausencia de \textit{aliasing de
punteros}.
Cuando dos punteros pueden se\~nalar regiones solapadas en memoria,
el compilador debe asumir que escribir a trav\'es de uno puede
modificar el valor le\'ido a trav\'es del otro, lo que impide la
reordenaci\'on de cargas y almacenamientos que la vectorizaci\'on
requiere.
El calificador \texttt{\_\_restrict} —extensi\'on est\'andar
adoptada por los compiladores C/C++ modernos, incluido
\texttt{icpx}--- permite al programador garantizar expl\'icitamente
que los punteros as\'i decorados no comparten ninguna posici\'on
de memoria con otros punteros dentro del \'ambito de la funci\'on.
En el kernel Prewitt, los arreglos $G_x$, $G_y$ y $\textit{Mag}$
son distintos por construcci\'on, de modo que declarar los tres
par\'ametros con \texttt{const float* \_\_restrict} y
\texttt{float* \_\_restrict} es sem\'anticamente correcto y
constituye la condici\'on m\'inima que habilita la vectorizaci\'on
autom\'atica en la versi\'on~2 (Autom\'atica).
...
La fase \textbf{Tripcounts + FLOP} cuantifica la carga real de
trabajo del programa completo.
Advisor instrumenta cada bucle del Survey e intercala contadores
que registran, durante la ejecuci\'on: el n\'umero real de
iteraciones (\textit{trip count}), las FLOPs ejecutadas
---distinguiendo entre operaciones SIMD empaquetadas y escalares---
y el volumen de datos transferidos desde cada nivel de la jerarqu\'ia
de memoria (L1, L2, L3, DRAM).
A partir de estos contadores se calcula la \textbf{intensidad
aritm\'etica medida} del programa seg\'un la ec.~\ref{eq:ai_medida}:

\begin{equation}
  \mathcal{I}_{\text{medida}} =
  \frac{\text{FLOPs totales}}{\text{Bytes transferidos a/desde memoria}}
  \label{eq:ai_medida}
\end{equation}

Este valor es un promedio del \emph{programa completo}, no del
kernel aislado.
Las operaciones de I/O de imagen PGM, c\'alculo de gradientes
Prewitt, replicaci\'on de datos y escritura del resultado
aportan menor densidad aritm\'etica que el kernel de magnitud,
diluyendo $\mathcal{I}_{\text{medida}}$ notablemente por debajo
del valor te\'orico del kernel puro.
Esta discrepancia es un resultado esperado que el modelo Roofline
permite diagnosticar al comparar el punto de operaci\'on medido
con los techos te\'oricos del hardware.
