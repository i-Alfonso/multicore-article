...
Autom\'atica & Vec.(Body)       & AVX2 & 3{,}276{,}800 & 16 \\
Guiada      & Vec.(Body)       & AVX2 & 6{,}553{,}600 &  8 \\
Expl\'icita  & Vec.(Body)       & AVX2 & 1{,}638{,}400 & 32$^*$ \\
\bottomrule
\end{tabular}%
}
\par\vspace{2pt}
{\scriptsize
$^*$~Unroll $4\times$ del compilador; ancho efectivo $= 8 \times 4 = 32$.\quad
$^{**}$~\texttt{-O2} aplica unroll~$8\times$ aun con \texttt{-no-vec};
Advisor reporta $N/8 = 6{,}553{,}600$.}
\end{table}
...
\texttt{\_\_intel\_avx\_rep\_memset}, la rutina interna del
compilador para inicializaci\'on de memoria con instrucciones AVX.
Esto confirma que la mayor parte del tiempo vectorizado corresponde
a transferencias de datos y no a c\'omputo aritm\'etico,
consistente con el car\'acter \textit{memory-bound} del kernel.
Cabe destacar que las versiones Guiada y Expl\'icita producen
perfiles de Code Analytics visualmente id\'enticos: ambas son
kernels AVX2 \textit{memory-bound} que invocan la misma rutina
de transferencia, por lo que el \textit{bottleneck} es el mismo
independientemente del ancho vectorial (8 vs.\ 32~floats efectivos).
La diferencia entre ambas versiones se aprecia en el Survey
(trip count $6{,}553{,}600$ vs.\ $1{,}638{,}400$) y en los
GFLOPS ($5.97$ vs.\ $7.15$~GFLOPS).
