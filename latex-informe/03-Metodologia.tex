...
Las opciones de compilaci\'on para cada versi\'on se resumen en
la Tabla~\ref{tab:flags}.

\begin{table}[h]
\centering
\caption{Opciones de compilaci\'on por versi\'on.}
\label{tab:flags}
...
La versi\'on Escalar utiliza \texttt{-O2} en lugar de \texttt{-O3}
para evitar que el compilador aplique transformaciones agresivas
de bucles ---como desenrollado intensivo o \textit{software pipelining}---
que podr\'ian mejorar el rendimiento escalar m\'as all\'a de lo
que obtendr\'ia un programador t\'ipico sin vectorizaci\'on expl\'icita.
El flag \texttt{-no-vec} desactiva la vectorizaci\'on SIMD,
mientras que \texttt{-O2} preserva las optimizaciones escalares
est\'andar, lo que proporciona la l\'inea base m\'as representativa
y conservadora para medir la ganancia real de las estrategias SIMD.
